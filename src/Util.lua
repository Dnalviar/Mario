...
function GeneratePoleTiles(atlas,tilewidth,tileheight)
	local sheetHeight = 48 / tileheight
...
function GenerateFlagTiles(atlas,tilewidth,tileheight)
	local offset = 96
	local sheetHeight = 64 / tileheight
	local sheetWidth = 48 / tilewidth
	local sheetCounter = 1
	--local tileWidth = 16
	--local tileHeight = 14
	local spriteSheet = {}
	for y = 0, sheetHeight - 1 do
		for x = 0, sheetWidth - 1 do
			spriteSheet[sheetCounter] = love.graphics.newQuad((x*tilewidth)+offset, y*tileheight,
					tilewidth, tileheight, atlas:getDimensions())
			sheetCounter = sheetCounter + 1
		end
	end
